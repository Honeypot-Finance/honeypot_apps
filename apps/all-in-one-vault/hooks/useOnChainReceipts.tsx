...
export function OnChainReceiptsProvider({
  ...
  // Fetch supported tokens independently — no dependency on receipt scan
  const fetchSupportedTokens = useCallback(async () => {
    if (!publicClient) return;
    try {
      const contracts = KNOWN_TOKEN_ADDRESSES.map((t) => ({
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (weight > BigInt(0)) {
          active.push({ id: KNOWN_TOKEN_ADDRESSES[i], weight: String(weight) });
        }
      }
      setSupportedTokens(active);
    } catch {
      // Non-critical — token list stays empty, user can still view receipts
    }
  }, [publicClient]);

  // Fetch supported tokens on mount (independent of receipts)
  useEffect(() => {
      ...
      try {
        ...
        for (let i = 0; i < batches.length; i += MAX_CONCURRENT_BATCHES) {
          ...
          for (const { start, results } of chunkResults) {
            parseReceiptBatch(start, results as any, lowerAddress, allResults);
          }
        }
  ...
  }, [refetchNextId, refetchTotalWeight, fetchSupportedTokens, fetchReceipts]);
