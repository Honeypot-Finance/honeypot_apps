...
const SwapPage = observer(() => {
  ...
  return isInit ? (
    <div className="w-full flex flex-col items-center pb-6 sm:pb-12 pt-8 px-2 sm:px-4 md:px-8 gap-8">
      <motion.div
        variants={itemPopUpVariants}
        initial="hidden"
        animate="visible"
        className="relative w-full max-w-[540px] flex flex-col items-center justify-start overflow-visible"
      >
        <Tabs.Root
          defaultValue="swap"
          value={activeTab}
      ...
      {activeTab === 'limit' && (
        <motion.div
          ...
          className="w-full xl:max-w-[1200px] 2xl:max-w-[1500px]"
        >
          <LimitOrderHistory
